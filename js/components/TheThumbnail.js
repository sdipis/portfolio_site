export default {
    ...
    <div class="bio-panel tooltip" :id="[piece.title]" :class="[piece.type]">
    <a :href='"project.php?id="+piece.id' target="_blank" :title='"Learn More About "+piece.title'>
    <p   class="tooltiptext">{{piece.title}}</p></a>
    <img  @click="showmydata" loading="lazy" :src='"dist/" + piece.display'>
    </div>

    
    `,
    
    // add this to div bio-panel to revert to lightbox @click="showmydata" 
    // <a :href='"project.php?id="+piece.id' target="_blank" :title='"Learn More About "+piece.title'>
